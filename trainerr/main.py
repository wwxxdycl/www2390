...
if __name__ == '__main__':
    ...
    parser.add_argument("--devices", type=str, default='cuda:0,cuda:1', help="Comma separated list of devices to use")
    
    parser.add_argument("--pretrained", type=str, default='/home/Bigfive/pre_model/roberta-base',
                        help="Pretrained Transformer Model")
    parser.add_argument("--checkpoint", type=str, default=f'../ckpt/{file_name}/checkpoint_6.pt')
    parser.add_argument("--tokenizer_name", type=str, default='/home/Bigfive/pre_model/roberta-base')
    parser.add_argument("--sbert", type=str, default='/home/Bigfive/pre_model/paraphrase-distilroberta-base-v1')  # 用于生成语言的近义词或近义句，是一个轻量级的RoBERTa模型
    parser.add_argument("--mode", type=str, default='train',
                        help="train/test")
    args = parser.parse_args()  # 解析命令行参数，并将解析结果存储在args对象

    device_list = args.devices.split(',') # 解析设备参数
    args.device = device_list  # 将设备列表赋值给 args.device
    ...
    if args.mode == "train":
        ...
        for fold in range(1, 3):
            print("fold: ", fold)
            train_data_path = f'/home/new_datasets_tf/{file_name}/fold_{fold}/train.json'
            test_data_path = f'/home/new_datasets_tf/{file_name}/fold_{fold}/test.json'
